...
| P9-54 | Linux 下 Release 构建脚本修正 | Done | 删除 `gradle.properties` 中硬编码的 Windows 专用 `org.gradle.java.home`（该行导致 Linux 上 Gradle 无法找到 JVM），JVM 改由调用方/脚本通过 `JAVA_HOME` 提供；新增与 `build-release.bat` 行为一致的 Linux 版 `build-release.sh`：`set -euo pipefail`，自动探测 `JAVA_HOME`（环境变量 → `/usr/lib/jvm/java-17-openjdk` 等候选）与 `ANDROID_HOME`（环境变量 → `~/Android/Sdk`），默认顺序构建 `armeabi-v7a` + `arm64-v8a`，支持单 ABI 参数与白名单校验，产物复制到 `~/.gradle/bilitv-native-build/release-apks/`；`gradlew` 补充可执行权限；README 构建章节、DEVELOPMENT_PLAN 与 AGENTS.md 验证命令补充 Linux 写法；脚本实跑成功，`aapt dump badging` 分别确认两个 APK 仅包含 `armeabi-v7a` / `arm64-v8a` native code |
